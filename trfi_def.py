...
def prepare_data():
    global keywords, keywordsth
    with open(f'keywords.txt', 'r', encoding='utf-8') as f:
        keywords = set(unidecode(l.rstrip('\n').lower()) for l in f)
    with open(f'keywords-th.txt', 'r', encoding='utf-8') as f:
        keywordsth = set(unidecode(l.rstrip('\n').lower()) for l in f)
...
class Base:
    ...
    def start_task2(self, i, item_url, is_running, first_run, site=None):
        if not is_running:
            ...
            if site not in (3, 22): self.prepare_inserts_to_db()
            return 'stop'
        if self._tpage == 'custom' and i == self.end_page:
            ...
            if first_run:
                write_old_item(i, self.start_page, self.item_url_for_compare, self.type_)
                if i == self.end_page:
    ...
    def validate_phone(self, phones, title, desc, title_url):
        is_invalid = False
        try:
            phones = list(dict.fromkeys(re.split('[^[0-9]+', phones.replace(' ', '').replace('.', ''))))
            for i, phone in enumerate(phones):
                ...
                if not phone.startswith('0'):
                    phone = '0' + phone
                    phones[i] = phone
                if phone.startswith(('03', '05', '07', '08', '092')):
                    logging.info(f'Dau so moi gioi: {phone}')
                    is_invalid = True
    ...
    def prepare_inserts_to_db(self):
        # Insert to db
    ...
    def check_key(self, title, desc, title_url):
        desc = unidecode(f'{title} {desc}'.lower())
        for key in keywords:
            if key in desc:
                logger.info('Moi gioi check key: %s - %s', key, title_url)
                self.count_moigioi += 1
                return True
        for key in keywordsth:
            keys = key.split(' - ')
            for k in keys:
                if k not in desc: break
            else:
                logger.info('Moi gioi check key to hop: %s - %s', key, title_url)
                self.count_moigioi += 1
                return True
...
def write_old_item(i, start_page, item_url, type_, site=None):
    print('i: ', i)
    print('start_p: ', start_page)
    if i == start_page:
...
def request_get(url, headers=None, data=None, timeout=60):
    if not headers:
        headers = {
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.132 Safari/537.36'}
    for i in range(3):
        try:
            response = rq.get(url, headers=headers, data=data, timeout=timeout)
            if response.status_code != 200:
                logger_err.error(f'Lỗi request đến server: {response.status_code} - {url}')
                time.sleep(3)
        except requests.exceptions.ConnectTimeout:
            logger_err.error(f'Request timeout: {url}')
            time.sleep(3)
        except Exception:
            logger_err.error(f'Request error: {url}')
            time.sleep(3)
        else:
            return response
    return None
...
def subStr(string, sub, length):
    id = string.index(sub) + len(sub)
